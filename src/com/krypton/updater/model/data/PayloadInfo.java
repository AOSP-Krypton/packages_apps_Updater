...
public final class PayloadInfo {
    ...
    public void extractPayloadInfo(Uri uri) {
        if (uri == null) {
            return;
        }
        filePath = uri.toString();
		try (ZipFile zip = new ZipFile(uri.getPath())) {
            // Set offset and size of payload
            setOffsetAndSize(zip);
    ...
    public boolean validateData() {
        if (filePath == null || offset == -1 || size == -1) {
            return false;
        }
    ...
    private void setOffsetAndSize(ZipFile zip) {
    ...
    private void setHeader(ZipFile zip) {
